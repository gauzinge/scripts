...
TH1D* cmnTest(int nEvents, double noiseSigma, double cmnSigma, std::string histoname, TH1D* thresholds)
{
	...
	for (int i = 0; i < nEvents; ++i) 
	{
		...
		if (histoname == "h_n_hits_dut_t_A") first_strip = 0, last_strip = 127;
		if (histoname == "h_n_hits_dut_t_B") first_strip = 127, last_strip = 254;
		if (histoname == "h_n_hits_dut_b_A") first_strip = 0, last_strip = 127;
		if (histoname == "h_n_hits_dut_b_B") first_strip = 127, last_strip = 254;
		
		if (histoname == "h_n_hits_fix_t_A") first_strip = 0, last_strip = 127;
		if (histoname == "h_n_hits_fix_t_B") first_strip = 127, last_strip = 254;
		if (histoname == "h_n_hits_fix_b_A") first_strip = 0, last_strip = 127;
		if (histoname == "h_n_hits_fix_b_B") first_strip = 127, last_strip = 254;
